...
当你按照前文的教程，历经千辛万苦，终于安装上 Rocky Linux 后，你肯定会有一个问题：我的 C 盘呢？

其实，Linux 的很多逻辑以及文件的组织结构都是与 Windows 大不相同的，接下来我们就一起了解一下。
...
> 假设我们当前在目录`/foo/bar`下，那么`/foo/bar/.`就相当于`/foo/bar`，`/foo/bar/..`，就相当于`/foo`

如果一个路径以`/`开头，那代表这个目录是从根目录开始的，根目录就相当于 Windows 上的 C 盘，是存放系统的位置。

> 注：在 Windows 下也有一个隐藏的根目录，不过我们几乎访问不到也访问不了

你所看到的主文件夹（或者是用户目录），位于 `/home/你的用户名`（Linux 中 root 用户比较特殊，就是`/root`），相当于 `C:\Users\你的用户名`（MacOS：`/Users/你的用户名`）。在这里你可以放置你的个人文件和代码。
